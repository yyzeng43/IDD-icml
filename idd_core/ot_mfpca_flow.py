...
from simulation.com_simu.ot_mfpca_closed_flow import (process_phaseI,
                                                      process_phaseII)
...
def fit_ot_phaseI_for_rep(
    ...
    ot_export_root: Path,
    reg: float = 0.05,
    bary_mode: str = "empirical",   # {"empirical","gaussian","fixed_support_sinkhorn","flow"}
    ot_mode: str = "empirical",     # {"empirical","gaussian","flow"}
    n_bary: int | None = 512,
    # --- flow options (only used when bary_mode/ot_mode == "flow") ---
    flow_bar_cfg: dict | None = None,
    flow_ot_cfg: dict | None = None,
    flow_device: str | None = None,
    flow_seed: int = 0,
    flow_warm_start: bool = False,
    flow_cache_dir: str | Path | None = None,
    flow_bar_hidden: int = 32,
    flow_bar_blocks: int = 8,
    flow_ot_hidden: int = 64,
    flow_ot_blocks: int = 16,
    flow_multiscale: bool = False,
) -> OTFitPhaseI:
    """
    Phase-I only:
    ...
    Xb, wb, tangI, ThatI, paramsI, cloudsI, tan_norms_I = process_phaseI(
        ...
        bary_mode=bary_mode,
        ot_mode=ot_mode,
        flow_bar_cfg=flow_bar_cfg,
        flow_ot_cfg=flow_ot_cfg,
        flow_device=flow_device,
        flow_seed=flow_seed,
        flow_warm_start=flow_warm_start,
        flow_cache_dir=flow_cache_dir,
        flow_bar_hidden=flow_bar_hidden,
        flow_bar_blocks=flow_bar_blocks,
        flow_ot_hidden=flow_ot_hidden,
        flow_ot_blocks=flow_ot_blocks,
        flow_multiscale=flow_multiscale,
    )
...
def export_ot_phaseII_ic(
    ...
    # --- flow options (only used when ot_mode == "flow") ---
    flow_ot_cfg: dict | None = None,
    flow_device: str | None = None,
    flow_seed: int = 0,
    flow_warm_start: bool = False,
